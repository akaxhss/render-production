...
def checkout(request):
    ...
    if user.role == User.CLIENT or user.role == User.HOSPITAL_MANAGER:
        ...
        if razorpay_payment_id and razorpay_subscription_id and razorpay_signature is not None:
            ...
            generated_signature = hmac.new(bytes(settings.RAZOR_API_ID, encoding='utf-8'),
                                           (razorpay_payment_id + "|" + payment.sub_Id).encode('utf-8'),
            ...
            if generated_signature == razorpay_signature:
                ...
                Subscriptions.objects.create(customer=payment.customer, valid_till=make_aware(validity),
                ...
                payment.delete()
                return JsonResponse({"Error": "Payment signature verification failed."},
                                    status=status.HTTP_400_BAD_REQUEST)
        else:
            return JsonResponse({"error": "razorpay_payment_id/razorpay_subscription_id/razorpay_signature is none"},
                                status=status.HTTP_400_BAD_REQUEST)
    else:
...
def get_all_plans(request):
    context = {'plans': []}
    response = requests.get('https://api.razorpay.com/v1/plans',
                            auth=HTTPBasicAuth(settings.RAZOR_KEY_ID, settings.RAZOR_API_ID))
